...
### 2) 渠道管理
- `GET    /api/v1/admin/channels`（分页列表：`page`、`page_size`、`keyword`）
...
- `POST   /api/v1/admin/channel/:id/models/refresh`（按当前已保存的渠道配置实时拉取上游模型，并刷新到 `channel_models`）
...
用于创建/编辑渠道时，根据当前已保存的渠道配置，请求该渠道的 `/models` 接口，并把结果同步到 `channel_models`。
...
### 2.1) 供应商管理
- `GET  /api/v1/admin/provider`（读取数据库中的供应商目录）
- `PUT  /api/v1/admin/provider`（保存供应商目录）

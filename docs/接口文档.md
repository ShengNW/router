...
- `POST   /api/v1/admin/channel/preview/capabilities`（基于草稿渠道与已选模型探测 chat/responses/images/audio 能力）
...
用于新建渠道流程中，在模型选择之后对渠道能力做一次真实探测。

请求体示例：
```json
{
  "protocol": "openai",
  "draft_id": "cad8bd65524a4d0c8f345a8c86ea9685",
  "models": ["gpt-4o-mini", "dall-e-3", "tts-1"]
}
```

响应体中的 `data.results` 为能力探测结果列表，典型字段包括：
- `capability`：能力标识，如 `chat`、`responses`、`images`、`audio`
- `endpoint`：本次探测的接口路径
- `model`：用于探测的模型
...
### 2.1) 模型供应商管理
- `GET  /api/v1/admin/provider`（读取数据库中的模型供应商目录）
- `PUT  /api/v1/admin/provider`（保存模型供应商目录）
- `GET  /api/v1/admin/provider/defaults`（读取系统预置的 12 家主流供应商默认目录）
- `POST /api/v1/admin/provider/fetch`（通过供应商 OpenAI 兼容 API 拉取模型并按供应商过滤）

说明：服务启动时 migration 会自动确保 `providers` 表已初始化，并补齐预置的 12 家主流供应商。

#### /api/v1/admin/provider
用于读取/保存供应商模型目录（可手动编辑）。
数据存储：`providers` 表（不再使用 `options` 的 `ModelProviderCatalog` 键）。
...
- `GET    /api/v1/admin/group/:name/channels`（读取分组绑定渠道）
- `PUT    /api/v1/admin/group/:name/channels`（更新分组绑定渠道）
